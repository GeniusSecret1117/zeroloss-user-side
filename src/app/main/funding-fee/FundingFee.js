...
import FundingProfitCard from "./components/fundingProfitCard";
import FundingLossCard from "./components/fundingLossCard";
...
function FundingFee() {
  const { t } = useTranslation("fundingFeePage");
  const orderdata = useSelector((state) => state.binCap.fundingFee) || [];
  const income = useSelector((state) => state.binCap.income) || [];
  ...
  income.map((item, index) => {
    if (item.income >= 0) {
      fundingProfit = fundingProfit + item.income;
    } else {
      fundingLoss = fundingLoss - item.income;
    }
  });
  const realizedProfit = fundingProfit - fundingLoss || 0;
  console.log("rp", realizedProfit);
  ...
  const handleSearchByPeriod = async () => {
    ...
    await BinService.getPeriodFundingHistory(startTime, endTime)
      .then((response) => {})
      .catch((err) => {});
  };
      ...
      content={
        <div className="w-full container p-12">
          ...
          <div className="flex justify-between mt-[20px] items-center flex-wrap gap-8">
            ...
            <div className="flex gap-8 flex-wrap">
              ...
              <div className="flex gap-8 items-center">
                ...
                <SearchButton onClick={handleSearchByPeriod}>
                  Search
                </SearchButton>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-[24px]">
            <FundingProfitCard value={formatNumber(fundingProfit)} />
            <FundingLossCard value={formatNumber(fundingLoss)} />
            <RealizedFundCard value={formatNumber(realizedProfit)} />
          </div>
          <div className="mt-[24px]">
            <OrderHistoryTable props={orderdata} />
          </div>
        </div>
      }
      scroll="content"
